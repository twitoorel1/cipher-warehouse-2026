...
CREATE TABLE `refresh_tokens` (
  ...
  CONSTRAINT `chk_refresh_tokens_window` CHECK ((`expires_at` >= `issued_at`))
...
CREATE TABLE `tel100_voice_profile` (
  ...
  CONSTRAINT `chk_tv_hub_password_aes` CHECK ((((`hub_password_ct` is null) and (`hub_password_iv` is null) and (`hub_password_tag` is null) and (`hub_password_kv` is null)) or ((`hub_password_ct` is not null) and (`hub_password_iv` is not null) and (`hub_password_tag` is not null) and (`hub_password_kv` is not null)))),
  CONSTRAINT `chk_tv_ptt_group_aes` CHECK ((((`ptt_group_ct` is null) and (`ptt_group_iv` is null) and (`ptt_group_tag` is null) and (`ptt_group_kv` is null)) or ((`ptt_group_ct` is not null) and (`ptt_group_iv` is not null) and (`ptt_group_tag` is not null) and (`ptt_group_kv` is not null)))),
  CONSTRAINT `chk_tv_sim_black_aes` CHECK ((((`sim_black_ct` is null) and (`sim_black_iv` is null) and (`sim_black_tag` is null) and (`sim_black_kv` is null)) or ((`sim_black_ct` is not null) and (`sim_black_iv` is not null) and (`sim_black_tag` is not null) and (`sim_black_kv` is not null))))
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
...
-- Dump completed on 2026-01-25  0:12:29
